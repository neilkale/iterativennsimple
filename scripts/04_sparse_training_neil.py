...
def main():
    ...
    # ic(dense_model)
    dense_summary = summary(dense_model)
    print()

    SPARSITY = 0.2
    sparse_model = SparseModel(num_in_features, num_hidden_features, num_out_features, SPARSITY)
    # ic(sparse_model)
    sparse_summary = summary(sparse_model)
    print()


    low_rank_model = LowRankModel(num_in_features, num_hidden_features, num_out_features, SPARSITY)
    # ic(low_rank_model)
    low_rank_summary = summary(low_rank_model)
    print()

    print("Dense Model:",end='\n')
    dense_train_losses, dense_val_losses, dense_times = train_model_with_stats_and_validation(dense_model, train_data, validation_data, num_in_features, num_out_features, 
                                                                                              MAX_EPOCHS = 100000, MAX_TIME = 500, LOG_EVERY_N_EPOCHS=25)
    print()

    print("Sparse Model:",end='\n')
    sparse_train_losses, sparse_val_losses, sparse_times = train_model_with_stats_and_validation(sparse_model, train_data, validation_data, num_in_features, num_out_features, 
                                                                                                 MAX_EPOCHS = 100000, MAX_TIME = 500, LOG_EVERY_N_EPOCHS=25)
    print()

    print("Low Rank Model:",end='\n')
    low_rank_train_losses, low_rank_val_losses, low_rank_times = train_model_with_stats_and_validation(low_rank_model, train_data, validation_data, num_in_features, num_out_features, 
                                                                                                       MAX_EPOCHS = 100000, MAX_TIME = 500, LOG_EVERY_N_EPOCHS=25)
    print()
...
def train_model_with_stats_and_validation(model, train_data, validation_data, num_in_features, num_out_features, MAX_EPOCHS = 100, LR = 0.001, MAX_TIME = 20, LOG_EVERY_N_EPOCHS = 1):
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=100, shuffle=True)
