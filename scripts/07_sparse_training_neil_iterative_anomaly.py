...
def main():
    ...
    models = [DenseIterativeNN, SparseIterativeNN, LowRankIterativeNN]
...
def train_model(map, train_data, features_list, is_iterative: bool, validation_data = None):
    ...
    sequence_length, iterations = train_data.z_start.shape[1], len(features_list)-1 + train_data.z_start.shape[1]
        ...
        for batch_idx, (start, target) in enumerate(train_loader):
            ...
            if is_iterative:
                for i in range(iterations):
                    if i < sequence_length: mapped = torch.cat((start[:, i, :features_list[0]], mapped[:, features_list[0]:]), dim = 1)
                    mapped = map(mapped)
...
def read_data(hidden_sizes, max_num_samples = np.Inf):
    ...
    # Read the data
    raw_data = json.load(open(DATA_DIR+'RandomAnomaly_Validate.json'))
    print("Completed.")
    # Data preprocessing to Torch tensors
    z_start_tensor = torch.tensor(np.vstack([[np.array(  sample[1][:-1]   )] for sample in raw_data])).float() # 100 x 11 x 2500 
    z_target_tensor = torch.tensor(np.vstack([[np.array(  sample[0][:-1]   )] for sample in raw_data])).float() # 100 x 11
    ...
    output_size = len(z_target_tensor[0])
    input_size = len(z_start_tensor[0][0])
    ...
    zh_start_tensor = torch.cat((z_start_tensor,
                                 torch.zeros(z_start_tensor.shape[0], z_start_tensor.shape[1], len(h_idx)), 
                                 torch.zeros(z_start_tensor.shape[0], z_start_tensor.shape[1], len(y_idx))),
                                 dim=2)
    zh_target_tensor = torch.cat((torch.zeros(z_target_tensor.shape[0], len(x_idx)),
...
def initialize_wandb(model: torch.nn.Module):
    wandb.init(
        project = "Iterative MNIST Anomaly",
        entity  = "neil-kale",
...
def get_loss_and_accuracy(map, data, features_list, is_iterative: bool):
    ...
    criterion = CRITERION
    sequence_length, iterations = data.z_start.shape[1], len(features_list)-1 + data.z_start.shape[1]
